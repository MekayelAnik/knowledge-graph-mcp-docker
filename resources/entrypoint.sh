...
# Validate and set MEMORY_PATH
validate_memory_path() {
    # Ensure MEMORY_PATH has a value
    MEMORY_PATH=${MEMORY_PATH:-$DEFAULT_MEMORY_PATH}
    
    # Validate the directory path
    if ! validate_directory "$MEMORY_PATH"; then
        echo "Invalid MEMORY_PATH: '$MEMORY_PATH'. Using default: $DEFAULT_MEMORY_PATH"
    ...
    if [ ! -d "$MEMORY_PATH" ]; then
        mkdir -p "$MEMORY_PATH"
        echo "Created memory directory: $MEMORY_PATH"
    fi
    
    # Set ownership if running as root
    if [ "$(id -u)" -eq 0 ]; then
        chown -R node:node "$MEMORY_PATH"
    fi
}

# Build MCP server command
build_mcp_server_cmd() {
    MCP_SERVER_CMD="npx -y mcp-knowledge-graph --memory-path \"$MEMORY_PATH\""
...
generate_client_config_example() {
    echo ""
    echo "=== AUTOAPPROVE CLIENT CONFIGURATION EXAMPLE ==="
    echo "To enable auto-approval in your MCP client, add this to your configuration:"
    echo ""
    echo "\"autoapprove\": ["
    echo "  \"aim_create_entities\","
    echo "  \"aim_create_relations\","
    echo "  \"aim_add_observations\","
    echo "  \"aim_search_nodes\","
    echo "  \"aim_read_graph\","
    echo "  \"aim_open_nodes\","
    echo "  \"aim_list_databases\""
    echo "]"
    echo ""
    echo "=== END AUTOAPPROVE EXAMPLE ==="
    echo ""
}
...
main() {
    ...
    [[ -n "${MEMORY_PATH:-}" ]] && MEMORY_PATH=$(trim "$MEMORY_PATH")
    ...
    case "${DEBUG_MODE:-}" in
        ...
        *)
            # Normal execution
            echo "Launching Knowledge Graph MCP Server with protocol: $PROTOCOL_DISPLAY on port: $PORT"
            echo "Memory path: $MEMORY_PATH"
            
            # Display authentication status
            if [[ -n "$API_KEY" ]]; then
                echo "API_KEY authentication is ENABLED for the Knowledge Graph MCP server"
            else
